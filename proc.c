...
void delete_node(struct skiplist *skiplist, int pid, int virtual_deadline)
{
  ...
  while (current_level >= 0)
  {
    while (current_node->virtual_deadline < virtual_deadline && current_node->pid != pid)
    { // TO DO: handle null case
...
int nicefork(int nice_value)
{
  ...
  *np->tf = *curproc->tf;
  ...
  insert_node(skiplist, np->pid, np->virtual_deadline);
        ...
        else
        {
          ...
          for (int k = 0; k <= highest_idx; k++)
          {
            ...
            if (pp->state == UNUSED)
              ...
              cprintf("[%d]%s:%d:%d(%d),", pp->pid, pp->name, pp->state, pp->nice_value, pp->virtual_deadline);
          }
          cprintf("\n");
        }
      }
